 ...
 * 2.把最大堆元素移动至最后为有序
 */
abstract public class HeapSort<T> implements Sort<T> {

    public void sort(T[] data, int k) {
    ...
    private  void maxHeapify(T[] data, int heapSize, int index) {
        ...
        int largest = index;
        if (left < heapSize && largeTo(data[left],data[index])) {
            largest = left;
        }
        if (right < heapSize && largeTo(data[right],data[largest])) {
            largest = right;
        }
    ...
    abstract public boolean largeTo(T a, T b);
